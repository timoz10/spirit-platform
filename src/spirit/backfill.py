...
def run(
    ...
    *,
    ...
    """Stream the CSV and POST chunks. Returns process exit code.
    ...
    print(f"Streaming {path} as ({pair}, {interval}m) → {api_url}/ohlc/upload")
    try:
        ...
        # Capability denial surfaces here as CapabilityDeniedError (HTTPError
        # subclass). Anything else is HTTP / network / pydantic.
        ...
        if isinstance(e, CapabilityDeniedError):
            print(
                ...
                f"Your key is probably Free or missing write:ohlc_user.",
                file=sys.stderr,
            )
            return 3
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        ...
        description=(
            "Bulk-upload OHLC from a Kraken CSV export to your scoped cloud "
            "store. Safe to run while Spirit is running — concurrent live "
            "pushes and historical uploads dedupe via ON CONFLICT."
        ),
    )
    ...
    parser.add_argument(
        "--api-url",
        default=os.environ.get("SPIRIT_API_URL", "https://api.tradebot.live/v1"),
        help="gateway URL (default: $SPIRIT_API_URL or api.tradebot.live)",
    )
    parser.add_argument(
        "--api-key",
        default=os.environ.get("SPIRIT_API_KEY", ""),
        help="API key (default: $SPIRIT_API_KEY)",
    )
    parser.add_argument(
    ...
    if not args.api_key:
        print(
            "ERROR: no API key. Set SPIRIT_API_KEY env var or pass --api-key.",
            file=sys.stderr,
        )
        return 2
