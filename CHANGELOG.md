...
- **Plus backtests no longer hammer the gateway (rate-limit/429).** In a Plus replay the cursor-bounded read wrapper fetched OHLC from the gateway on *every* strategy eval — hundreds of calls per run — tripping the rate limiter and silently dropping candles. It now fetches each `(pair, interval)` series once and serves cursor-bounded slices from memory. (#830 follow-on)
- **Replay run register/finalize stopped logging spurious warnings on the wheel.** `run_manager.register_run`/`finalize_run` import the unbundled `db_connection` (the dev-only `replay_runs` archive table); on the wheel this raised, surfacing a `[RUN] Failed to …` WARNING + traceback. They now degrade to a debug no-op when `db_connection` is absent — run listing already routes via the DataProvider Protocol. (#830 follow-on)
- **Open positions now persist the instant they open or close.** On the legacy (non-Spine) evaluation path the only `spirit_state` save ran at the *start* of each cycle, before the trade logic — so a position opened mid-cycle wasn't persisted until the next candle (up to a full interval later), and a crash in that window restored flat (losing a live position). Entries and exits now trigger an immediate, event-driven save. (#835)
...
- **Paper trades silently failed to persist to `strategy_performance`.** rc2 surfaced this on the first completed ETHUSD paper trade: `paper_order_executor._record_to_pg` lazy-imported `record_trade` from `spirit.indicators.decision_engine.engine.strategy_performance_writer` — a path explicitly excluded from the public mirror. Every `pip install` (Free or Paid) hit `ModuleNotFoundError`, caught silently by `except Exception`, and the trade's audit row was never written. **The trade itself fired correctly and equity tracked**; only the audit was lost. Same bug existed in rc1, but rc1 ran 14h without an SMA trade firing. Fix: relocated the writer to `src/spirit/utils/strategy_performance_writer.py` (its rightful home — pure DataProvider indirection with zero decision-engine logic, satisfying Rule 4 for central-table writers). Updated all 6 importer paths (`paper_order_executor`, `order_executor`, 3 internal scripts, 1 test). Tightened the exception handler to split `ImportError`/`ModuleNotFoundError` (packaging bug) from runtime errors; in `--mode test` the packaging error re-raises so CI install-smoke catches it, in `--mode paper`/`--mode live` it logs loudly with traceback rather than crashing the trading loop. Live-mode handler never hard-fails on this path (position-holding trader must not crash on a packaging bug). New install-smoke gate in `tests.yml` asserts `from spirit.utils.strategy_performance_writer import record_trade` resolves on the pipx-installed wheel. (#803)
- **BYOD CSV import was absent from the installed wheel.** The headline v2.2.4 feature — `python3 -m spirit.backfill <kraken-csv>` loading OHLC into local SQLite — was fully functional in source but three of its files were never added to the public-mirror allowlist: `src/spirit/backfill.py` (the CLI itself), `src/spirit/utils/kraken_csv.py` (the CSV parser), and `src/spirit/utils/paid_tier_composite.py` (Paid-tier OHLC routing). On a `pip install`, the standalone command didn't exist and the setup-wizard CSV prompt crashed at `from spirit.utils.kraken_csv import …` (`ModuleNotFoundError`). The Free-tier soak passed because it never exercised the CSV path from the *installed wheel*. Same fault class as #803 / v2.2.1. Fixed by adding the three files to the allowlist; the mirror dry-run confirms they now survive into the public tree.
- **Paid-tier paper/live crashed importing the IP-tier dlimit health check.** `main.py` unconditionally imported `spirit.indicators.d_limit_v3.health_check` when wiring the WS freshness cache — a path reached by any non-Free instance with the default `PIPELINE_EVENT_BUS=ws` plus an API key (the normal Paid config). `d_limit_v3` is IP and excluded from the wheel, so the import raised `ModuleNotFoundError`. Only the Free path (`not _free_tier` skips the block) was soaked, so it stayed hidden — found by the new allowlist-vs-imports audit below, not in production. Fixed by catching `(ImportError, ModuleNotFoundError)` explicitly and degrading gracefully: the freshness cache still records WS-pushed events, it just runs without a local health-check hook. (#803 lesson applied — name the cause, never `except Exception`.)
...
- **Five new install-smoke gates on every PR** (`tests.yml`):
  ...
  - `from spirit.utils.strategy_performance_writer import record_trade` resolves on the pipx-installed wheel (#803 contract — rc3)
...
- **New allowlist-vs-imports audit (Task #11) — the general fix for the #803 / v2.2.1 fault class.** `scripts/check_allowlist_imports.py` AST-walks every allowlisted module and fails if it imports an excluded `spirit.*` module (which would `ModuleNotFoundError` on the wheel), unless the import is guarded by `try/except (ImportError, ModuleNotFoundError)` or carries a documented, reviewed waiver in `docs/features/platform/MIRROR_IMPORT_WAIVERS.txt`. Runs as `tests/test_allowlist_imports_audit.py` in the per-PR pytest job. On adoption it surfaced two latent packaging crashes (BYOD CSV files + the Paid dlimit import above) and seven dev/daemon/debug-only sites, now waived with written reachability reasons. This is the third instance of "an allowlisted module imports a file that isn't in the wheel" (v2.2.1 `data_types`, #803 writer, this) — now CI-enforced so there isn't a fourth.
